...
class Schedule:
    ...
    def calculate_schedule(self):
        for i in self.trucks:
            ...
            for j in i.tasks:
                ...
                for k in range(0, len(j.route) - 1):
                    ...
                    for l in travel_append:
                        ...
                        if len(self.schedule) == 0:
                            ...
                            self.schedule = {l: [{'trucks': {i.id: np.array([j.route[k], j.route[k + 1],
                                                                             l / max(travel_append),
                                                                             state])}},
                                                 'C/R', dump_list, clea_list, bnch_list]}
                            ...
                            if l not in self.schedule:
                                ...
                                for a in self.schedule[l][0]['trucks']:
                                    ...
                                    if self.schedule[l][0]['trucks'][a][0:2].astype(int) in np.array([j.route[k], j.route[k + 1]]) and \
                                       self.schedule[l][0]['trucks'][a][3] == state and \
                                       (distance * (float(self.schedule[l][0]['trucks'][a][2])) < Truck.length * Truck.distance_policy / 1000):
                                        # if there are trucks overlapping append the distancing
                                        trucks_in_route = np.append(trucks_in_route,
                                                                    float(self.schedule[l][0]['trucks'][a][2]))
                                ...
                                if len(trucks_in_route) > 1:
                                    ...
                                    travel_time_displaced = int(
                                        ((Truck.length * Truck.distance_policy) / 1000 - min(trucks_in_route)) / int(
                                            i.avg_speed[i.model].loc[name + state]) * 3600)
                    ...
                    if self.graph.nodes[j.route[k + 1]].type in [NodeType.BNCH, NodeType.CLEA, NodeType.DUMP,
                                                                 NodeType.CRSH]:
                        ...
                        if index != -1:
                            ...
                            counter_trucks_w = self.get_fixed_times(j.route[k + 1]) * (1 - float(
                                self.schedule[travel_append[len(travel_append) - 1]][0]['trucks'][index][
                                    2]) + counter_trucks_w)

                            time_travel_delayed = distance / int(
                                i.avg_speed[i.model].loc[name + state] - self.speed_reduction) * 3600
                            ...
                            if time_travel_delayed < time_travel + counter_trucks_w:
                                ...
                                travel_append_delayed = np.arange(len(time_periods),
                                                                  int(time_travel_delayed / self.time_delta) + len(
                                                                      time_periods), self.time_delta)
                        ...
                        waiting_append = np.arange(len(time_periods),
                                                   int(counter_trucks_w / self.time_delta) + len(time_periods),
                                                   self.time_delta)
                        ...
                        if self.graph.nodes[j.route[k + 1]].type in [NodeType.BNCH, NodeType.CLEA]:
                            ...
                            for l in waiting_append:
                                ...
                                if l not in self.schedule:
                                    ...
                                    self.schedule[waiting_append[l]] = [{'trucks': {
                                        i.id: np.array(j.route[k], j.route[k + 1], l / max(waiting_append), 'W/LD')}},
                                                                        self.schedule[waiting_append[l - 1]][2],
                                                                        self.schedule[waiting_append[l - 1]][3],
                                                                        self.schedule[waiting_append[l - 1]][4]]
                            ...
                            for l in waiting_append:
                                ...
                                if l not in self.schedule:
                                    ...
                                    self.schedule[waiting_append[l]] = [{'trucks': {
                                        i.id: np.array(j.route[k], j.route[k + 1], l / max(waiting_append), 'W/ULD')}},
                                                                        self.schedule[waiting_append[l - 1]][2],
                                                                        self.schedule[waiting_append[l - 1]][3],
                                                                        self.schedule[waiting_append[l - 1]][4]]
                                    self.time_trck_idle = self.time_trck_idle + 1
                        ...
                        fixed_append = np.arange(len(time_periods),
                                                 self.get_fixed_times(j.route[k + 1]) + len(time_periods),
                                                 self.time_delta)
                        ...
                        if self.graph.nodes[j.route[k + 1]].type == NodeType.BNCH:
                            ...
                            for l in fixed_append:
                                ...
                                if l not in self.schedule:
                                    ...
                                    self.schedule[l] = [{'trucks': {
                                        i.id: np.array([j.route[k], j.route[k + 1], l / max(fixed_append), 'LD'])}},
                                        self.schedule[l - 1][1], self.schedule[l - 1][2],
                                        self.schedule[l - 1][3], self.schedule[l - 1][4]]
                                ...
                                if l not in self.schedule:
                                    ...
                                    self.schedule[l] = [{'trucks': {
                                        i.id: np.array([j.route[k], j.route[k + 1], l / max(fixed_append),
                                                        'LD'])}},
                                        self.schedule[l - 1][1], self.schedule[l - 1][2], self.schedule[l - 1][3],
                                        self.schedule[l - 1][4]]
        ...
        self.solution.time_idle = len(self.schedule) + self.time_crsh_idle
        self.solution.time_total = len(self.schedule)
